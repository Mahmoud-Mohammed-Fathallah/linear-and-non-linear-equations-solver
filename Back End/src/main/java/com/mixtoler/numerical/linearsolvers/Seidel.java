...
public class Seidel {
 ...
 double Es;
 int SF;
public  double[] SeidelCalc(double A[][],double B[], double x[], double in, double ES, int sf  ){
    a=A;
    b=B;
    X=x;
    I=in;
    ...
    for(int i=0; i<X.length; i++){
        ...
        for(int r=0; r<X.length; r++){
            if (r == i){} else{Row =  round ((  round (a[i][r]) * round (X[r]))+ round (Row)  ) ;} }
            
        Xold=X[i];
        X[i]=  round ( round ((b[i]) - round (Row))  / round (( a[i][i] ))  )  ;       //Get new X
        Er[i]=  Math.abs( ( X[i]- Xold )/ X[i] ) ;  // Get Er
    }
